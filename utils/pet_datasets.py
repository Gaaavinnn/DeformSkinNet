...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __len__(self):
        # with open('1234.txt','w') as fff:
        #     fff.write(str(self.img_files))
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                ...
                landmarks = np.array([xyn2xy(x, w, h, pad[0], pad[1]) for x in landmarks])
        ...
        if self.augment:
            # Augment imagespace
            if not mosaic:
                img, labels = random_perspective(img, labels, landmarks,
                                                 degrees=hyp['degrees'],
...
def load_mosaic(self, index):
    ...
    #for x in (labels4[:, 1:], *landmarks4):
    #    np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()
    np.clip(labels4[:, 1:], 0, 2 * s, out=labels4[:, 1:])
    # lan_id=0
    for index, landmark in enumerate(landmarks4):
        #print('\nlandmarks4\n',index, landmark)
        # if lan_id==0:
        #     landmark_last = landmark
        # if landmark.ndim==1:
        #     landmark = landmark_last
        #     lan_id+=1
        for k, point in enumerate(landmark):
            # print(point)
            x,y = point
            if x < 0 or y < 0 or x > 2*s or y > 2*s:
                #print(index,  ' -----  ', k)
                landmarks4[index][k] = [-1, -1]

    #print('landmarks4: ',landmarks4)

    # img4, labels4 = replicate(img4, labels4)  # replicate

    # Augment
    img4, labels4 = random_perspective(img4, labels4, landmarks4,
...
def load_mosaic9(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            ...
            landmarks = [xyn2xy(x, w, h, padx, pady) for x in landmarks]
        labels9.append(labels)
        landmarks9.extend(landmarks)
    ...
    landmarks9 = [x - c for x in landmarks9]

    for x in (labels9[:, 1:], *landmarks9):
        np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()
    # img9, labels9 = replicate(img9, labels9)  # replicate

    # Augment
    img9, labels9 = random_perspective(img9, labels9, landmarks9,
                                       degrees=self.hyp['degrees'],
                                       translate=self.hyp['translate'],
                                       scale=self.hyp['scale'],
                                       shear=self.hyp['shear'],
...
def random_perspective(img, targets=(), landmarks=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
                       border=(0, 0)):
    ...
    n = len(targets)
    if n:
        use_landmarks = any(x.any() for x in landmarks)
        landmarks = np.array(landmarks)
        #new = np.zeros((n, 4))
        if use_landmarks:  # warp segments
            #landmarks = resample_segments(landmarks)  # upsample
            mask_landmarks = (landmarks != -1)
            for i, landmark in enumerate(landmarks):
                xy = np.ones((len(landmark), 3))
                #print('landmark',type(list(landmark)),list(landmark),type(landmark.ndim))
#                 if landmark.ndim==1:
...
# [        -1   ,      -1]])
        ...
        # filter candidates
        i = box_candidates(box1=targets[:, 1:5].T * s, box2=new.T, area_thr=0.01 if use_landmarks else 0.10)
        targets = targets[i]
        if use_landmarks:
