...
class Conv(nn.Module):
    ...
    def forward(self, x):
        # print("self.act(self.bn(self.conv(x))", self.act(self.bn(self.conv(x))).shape)
...
class C3(nn.Module):
    ...
    def forward(self, x):
        ...
        #print('C3(nn.Module):',x.shape,self.cv1, self.cv2, self.cv3)
...
class SPP(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.cv2(torch.cat([self.cv1(x)] + [m(self.cv1(x)) for m in self.m], 1))
...
class DeformConv2D(nn.Module):
    def __init__(self, inc, outc, kernel_size=3, padding=1, bias=None):
        ...
        self.conv_kernel = nn.Conv2d(inc, outc, kernel_size=7,stride=1 ,padding=0)
    ...
    def forward(self, x):
        ...
        mask = torch.cat([p[..., :N].lt(self.padding)+p[..., :N].gt(x.size(2)-1-self.padding),
                          p[..., N:].lt(self.padding)+p[..., N:].gt(x.size(3)-1-self.padding)], dim=-1).type_as(x)
        mask = mask.detach()
        ...
        # print('x_q_lt',x_q_lt.type())
        # (b, c, h, w, N)
        x_offset = g_lt.unsqueeze(dim=1) * x_q_lt + \
                   g_rb.unsqueeze(dim=1) * x_q_rb + \
                   g_lb.unsqueeze(dim=1) * x_q_lb + \
                   g_rt.unsqueeze(dim=1) * x_q_rt
       # print("x_offset.shape",x_offset.shape,ks)
        x_offset = self._reshape_x_offset(x_offset, ks)

        
        x_offset = self.pool(x_offset)
        out = self.conv(x_offset)
        out = F.relu(out)
        #print("out3",out.shape, out.type())
